...
pub struct LintCommonOptions {
    ...
    /// Options for the `pyspark` plugin.
    #[option_group]
    pub pyspark: Option<PySparkOptions>,
...
#[serde(deny_unknown_fields, rename_all = "kebab-case")]
#[cfg_attr(feature = "schemars", derive(schemars::JsonSchema))]
pub struct PySparkOptions {
    /// The maximum `PySpark` complexity to allow before triggering `PS001` errors.
    #[option(
        default = "3",
        value_type = "int",
        example = r#"
            # Flag errors (`PS001`) whenever the complexity level exceeds 5.
            max-complexity = 5
        "#
    )]
    pub max_complexity: Option<usize>,
...
impl PySparkOptions {
    pub fn into_settings(self) -> pyspark::settings::Settings {
        pyspark::settings::Settings {
            max_complexity: self
                .max_complexity
                .unwrap_or(pyspark::settings::DEFAULT_MAX_COMPLEXITY),
        }
    }
}
